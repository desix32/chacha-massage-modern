        ...
        {isSuccess ? (
          <div className="modal-success-state">
            ...
            <div className="success-summary-box">
              <div><strong>Treatment:</strong> {currentTreatment.name} ({selectedDuration} min)</div>
          ...
          <form className="booking-form" onSubmit={handleWhatsAppSubmit}>
            <div className="modal-header">
              <div className="section-tag">Instant Reservation</div>
              <h2 className="modal-title">Book Your Sanctuary Session</h2>
              <p className="modal-desc">
                No deposit required. Instant confirmation directly via WhatsApp or phone.
              </p>
            </div>
            ...
            <div className="form-body">
              ...
              <div className="form-group">
                <label className="form-label">1. Select Branch Location</label>
              ...
              {/* Treatment Selector */}
              <div className="form-group">
                <label htmlFor="treatment-select" className="form-label">2. Select Therapeutic Treatment</label>
                <select
                  id="treatment-select"
                ...
                >
                  {TREATMENTS.map(t => (
                    <option key={t.id} value={t.id}>
                      {t.name} (from THB {t.durations[0].priceTHB})
                    </option>
                  ))}
                </select>
              </div>
              ...
              {/* Duration Pills */}
              <div className="form-group">
                <label className="form-label">3. Duration & Live Pricing</label>
                <div className="duration-options">
                  {currentTreatment.durations.map(dur => (
                    ...
                    >
                      <Clock size={14} />
                      <span className="dur-text">{dur.minutes} Mins</span>
              ...
              <div className="form-row-2">
                  ...
                  <div className="guests-toggle">
                    {[1, 2, 3].map(num => (
                      <button
                        key={num}
                        type="button"
                        className={`guest-opt-btn ${guestsCount === num ? 'active' : ''}`}
                        onClick={() => setGuestsCount(num)}
                      >
                        {num === 1 ? '1 Guest' : num === 2 ? 'Couple (2)' : '3+ Group'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="form-group">
                  <label className="form-label">Preferred Pressure</label>
                  <div className="pressure-toggle">
                    {['Soft', 'Medium', 'Strong'].map(p => (
                      <button
                        key={p}
                        type="button"
                        className={`pressure-btn ${pressureLevel === p ? 'active' : ''}`}
              ...
              <div className="modal-total-bar">
                <div>
                  <span className="total-label">Total Payable at Spa:</span>
                  <div className="total-amount">
                    <span>THB {totalPriceTHB.toLocaleString()}</span>
                    <small>({guestsCount} x {selectedDuration} mins)</small>
                  </div>
                </div>
                <div className="no-prepayment-badge">
                  <span>Pay after service • No deposit</span>
                </div>
              </div>
